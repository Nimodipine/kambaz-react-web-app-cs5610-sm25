...
export const findAssignmentsForCourse = async (cid: string) => {
    const response = await axiosWithCredentials.get(`${ASSIGNMENTS_API}/course/${cid}`);
    return response.data;
};
...
export const updateAssignment = async (assignment: any) => {
    const response = await axiosWithCredentials.put(`${ASSIGNMENTS_API}/${assignment.id}`, assignment);
    return response.data;
};
